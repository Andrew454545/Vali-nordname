...
public static class LocationDistributor
{
    ...
    public static async Task<(IList<T> locations, int minDistance)> WithMaxMinDistanceAsync<T, T2>(
        ...
        int? minMinDistance = null,
        Func<T, Task<bool>>? includePredicate = null) where T : IDistributionLocation<T2> where T2 : IComparable<T2>
    {
        ...
        do
        {
            ...
            var distributedLocations = (await DistributeEvenlyAsync<T, T2>(locations, minDistance, locationProbability, avoidShuffle: avoidShuffle, silent: true, locationsAlreadyInMap: locationsAlreadyInMap, includePredicate: includePredicate))
            ...
            static int NextMinDistanceIndex(Dictionary<int, IList<T>?> visitedLocations, int goalCount, int[] distances)
            {
                ...
                var visitedWithoutSatisfiedGoalCount = visitedLocations.Where(v => v.Value != null && v.Value.Count < goalCount).ToArray();
    ...
    public static async Task<IList<T>> DistributeEvenlyAsync<T, T2>(
        ICollection<T> locations,
        int minDistanceBetweenLocations,
        LocationProbability locationProbability,
        bool avoidShuffle = false,
        ...
        IReadOnlyCollection<ILatLng>? locationsAlreadyInMap = null,
        bool silent = false,
        Func<T, Task<bool>>? includePredicate = null) where T : IDistributionLocation<T2>, ILatLng where T2 : IComparable<T2>
    {
        var list = new List<(T loc, string hash)>();
        var precision = minDistanceBetweenLocations switch
        {
        ...
        if (silent)
        {
            await Distribute(null);
        }
        else
        {
            await AnsiConsole.Progress().StartAsync(Distribute);
        }

        return list.Select(x => x.loc).ToArray();

        async Task Distribute(ProgressContext? ctx)
        {
            ...
            foreach (var group in groups)
            {
                ...
                var alreadyInMap = ensureNoNeighborSpill ? list.Where(x => neighbors.Contains(x.hash)).Select(x => (ILatLng)x.loc).Concat(locationsAlreadyInMap ?? []).ToArray() : (locationsAlreadyInMap ?? []);
                var distributionLocations = group.ToArray();
                var selection = await GetSomeAsync<T, T2>(distributionLocations, 1_000_000, minDistanceBetweenLocations, locationProbability, locationsAlreadyInMap: alreadyInMap, avoidShuffle: avoidShuffle, includePredicate: includePredicate);
                list.AddRange(selection.Select(x => (x, ensureNoNeighborSpill ? Hasher.Encode(x.Lat, x.Lng, precision) : "")));
    ...
    public static async Task<IList<T>> GetSomeAsync<T, T2>(
        ICollection<T> locations,
        int goalCount,
        int minDistanceBetweenLocations,
        LocationProbability locationProbability,
        ...
        Func<T, Task<bool>>? includePredicate = null) where T : IDistributionLocation<T2> where T2 : IComparable<T2>
    {
        ...
        {
            foreach (var point in notProcessed.ToList())
            {
        ...
        {
            MapLoc<T2> randomPoint;
            if (randomizer != null)
            {
                randomPoint = notProcessed[randomizer.NextWithRemoval()];
            }
            else
            {
            ...
            var location = locationsLookup[randomPoint.LocationId];
            if (includePredicate != null && !await includePredicate(location))
            {
            ...
            foreach (var pointOnMap in notProcessed.Values.ToList())
            {
