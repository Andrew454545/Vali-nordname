...
public static class LocationDistributor
{
        ...
        Func<T, Task<bool>>? includePredicate = null) where T : IDistributionLocation<T2> where T2 : IComparable<T2>
    {
        ...
        var ordered = BuildOrderedCandidates<T, T2>(locations, locationProbability, avoidShuffle);
        var includeCache = new Dictionary<T2, bool>();

        async Task<bool> Included(T loc)
        {
            ...
            if (includeCache.TryGetValue(loc.LocationId, out var cached))
            {
                return cached;
            }

            var included = await includePredicate(loc);
            includeCache[loc.LocationId] = included;
            return included;
        }

        var evalCache = new Dictionary<int, (IList<T> result, int count)>();

        async Task<(IList<T> result, int count)> Eval(int idx)
        {
            if (evalCache.TryGetValue(idx, out var cached)) return cached;
            var placed = await PlaceSpacedAsync<T, T2>(ordered, goalCount, distances[idx], locationsAlreadyInMap, Included);
            var r = (placed, placed.Count);
            evalCache[idx] = r;
            return r;
        }
        ...
        while (low <= high)
        {
            var (result, count) = await Eval(probe);
            if (count >= goalCount)
            {
                bestResult = result;
                bestDistance = distances[probe];
    ...
    public static async Task<IList<T>> PlaceSpacedAsync<T, T2>(
        IReadOnlyList<T> orderedCandidates,
        int goalCount,
        int minDistanceBetweenLocations,
        IReadOnlyCollection<ILatLng>? locationsAlreadyInMap = null,
        Func<T, Task<bool>>? includePredicate = null)
        where T : IDistributionLocation<T2> where T2 : IComparable<T2>
    {
        if (goalCount <= 0 || orderedCandidates.Count == 0)
        {
        ...
        var cosRef = Math.Cos(Math.Min(maxAbsLat, 89.0) * Math.PI / 180.0);
        if (cosRef < 1e-6) cosRef = 1e-6;
        var cellLat = d / metresPerDegree;
        var cellLng = d / (metresPerDegree * cosRef);
        ...
        for (var i = 0; i < orderedCandidates.Count; i++)
        {
            ...
            var loc = orderedCandidates[i];
            if (includePredicate != null && !await includePredicate(loc))
            {
                continue;
            }
