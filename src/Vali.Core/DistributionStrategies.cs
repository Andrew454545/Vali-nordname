...
public static class DistributionStrategies
{
        ...
        string subdivision)
    {
        ...
        if (preferenceFilters.Any())
        {
            ...
            foreach (var locationPreferenceFilter in preferenceFilters)
            {
                ...
                var withMaxMinDistance = ByMaxMinDistanceWithRoadFilter(
                    filtered,
                    goalCount,
                    minMinDistance,
                    locationsAlreadyInMap,
                    locationProbability,
                    mapDefinition);
        ...
        return ByMaxMinDistanceWithRoadFilter(
            filteredLocations,
            regionGoalCount,
            minDistance,
            usedLocations,
    ...
    private static (IList<Loc> locations, int minDistance) ByMaxMinDistanceWithRoadFilter(
        ICollection<Loc> filteredLocations,
        int goalCount,
        int minDistance,
        IReadOnlyCollection<ILatLng>? locationsAlreadyInMap,
        LocationProbability locationProbability,
        MapDefinition mapDefinition)
    {
        if (!mapDefinition.RejectRoadName)
        {
            return LocationDistributor.WithMaxMinDistance<Loc, long>(
                filteredLocations,
                goalCount,
                minMinDistance: minDistance,
                locationsAlreadyInMap: locationsAlreadyInMap,
                locationProbability: locationProbability,
                avoidShuffle: GenerationDeterminism.Deterministic);
        }

        return LocationDistributor.WithMaxMinDistanceAsync<Loc, long>(
                filteredLocations,
                goalCount,
                minMinDistance: minDistance,
                locationsAlreadyInMap: locationsAlreadyInMap,
                locationProbability: locationProbability,
                avoidShuffle: GenerationDeterminism.Deterministic,
                includePredicate: async loc => !await GoogleApi.HasRoadName(loc.Google.PanoId ?? ""))
            .GetAwaiter()
            .GetResult();
    }
